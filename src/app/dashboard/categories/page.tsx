    ...
    <div className="relative aspect-[16/9] w-full bg-muted overflow-hidden text-left">
      ...
      {isActive && (
        <div className="absolute inset-0 z-10 pointer-events-none overflow-hidden">
          <div className="shine-sweep absolute top-0 -left-[100%] w-[40%] h-[200%] bg-gradient-to-r from-transparent via-white/40 to-transparent skew-x-[-30deg] -translate-y-1/4" />
        </div>
      )}
...
export default function ManageOutletsPage() {
  ...
  const kpiCards: StatCardData[] = useMemo(() => [
    { title: "Active Outlets", value: allOutlets.length.toString(), change: '+1', changeDescription: 'this week', icon: Store, color: 'orange' },
    { title: 'Digital Orders', value: '1,284', change: '+8%', changeDescription: 'vs last month', icon: TrendingUp, color: 'green' },
    { title: 'QR Menu Scans', value: '4,562', change: '+12%', changeDescription: 'vs yesterday', icon: QrCode, color: 'blue' },
    { title: 'Avg. Rating', value: '4.8', change: '+0.1', changeDescription: 'vs last month', icon: Star, color: 'purple' },
  ], [allOutlets.length]);

  if (isLoading) return <CategoriesPageSkeleton view="gallery" />;
  ...
  return (
    <>
        ...
        <div className="max-w-[1800px] mx-auto space-y-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="text-left">
              ...
              <p className="text-muted-foreground mt-1">Overview and configuration for all outlet locations</p>
            </div>
            <Button className="gap-2 font-bold bg-primary hover:bg-primary/90 text-primary-foreground" onClick={handleAddNewOutlet}>
              <Plus className="h-5 w-5" /> Add New Outlet
            </Button>
          </div>
          ...
          <div className="flex items-center gap-4 bg-background p-4 rounded-xl border shadow-sm">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search outlet location or name" className="pl-10 border-0 bg-transparent focus-visible:ring-0 text-base" value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>
            <Button variant="outline" className="gap-2 font-semibold border-muted"><SlidersHorizontal className="h-4 w-4" /> Sort</Button>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredOutlets.map((outlet) => (
              <OutletCard key={outlet.id} outlet={outlet} isActive={activeOutletId === outlet.id} onSelect={handleSelectOutlet} onQuickSettings={handleOpenQuickSettings} onEdit={handleEditOutlet} onDelete={handleDeleteOutlet} />
            ))}
          </div>
          ...
          <div className="pt-8 flex flex-col sm:flex-row items-center justify-between gap-4 border-t">
            <p className="text-sm text-muted-foreground">Showing <strong>1 to {filteredOutlets.length}</strong> of <strong>{allOutlets.length}</strong> outlets</p>
      ...
      <QuickSettingsSheet open={isQuickSettingsOpen} onOpenChange={setIsQuickSettingsOpen} restaurant={selectedOutlet ? { id: selectedOutlet.id, name: selectedOutlet.name, status: selectedOutlet.status } : null} />
    </>
  );
}
