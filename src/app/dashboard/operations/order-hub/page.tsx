          ...
          <div className="p-4 space-y-4">
            <div className="flex justify-between items-start">
              ...
              <div className={cn(
                ...
                {(isExiting && order.exitType ? exitConfig[order.exitType].text : (config as any).text)}
              </div>
            </div>
...
export default function OrderHubPage() {
  ...
  // Strict 10-second sync finalization cycle
  useEffect(() => {
      ...
      setOrders(prev => {
        ...
        if (randomExit === 'COMPLETED') {
            if (inPreparing.length > 0) {
              ...
              // If no preparing items, fallback to a cancellation/failure from any column
  ...
  return (
    <div className={cn("min-h-screen bg-[#fafbfc] flex flex-col", inter.className)}>
      <DashboardHeader />
      ...
      <main className="flex-1 p-8 relative overflow-visible">
        <div className="max-w-[1800px] mx-auto grid grid-cols-1 xl:grid-cols-4 gap-12">
          
          <div className="xl:col-span-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 min-w-0 h-full">
            {columns.map((col) => {
              ...
              return (
                <div key={col.id} className="flex flex-col min-w-0 h-full text-left">
                  <div className={cn("mb-6 rounded-2xl border border-slate-100 p-4 transition-all shadow-sm", config.bg)}>
          ...
          <aside className="xl:col-span-1 sticky top-[108px] self-start text-left h-fit z-30">
                      ...
                      {recentExits.length > 0 ? recentExits.map((event) => {
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-start gap-4">
                              <div className={cn(
                                "h-2 w-2 rounded-full mt-2.5 shrink-0 shadow-sm relative z-10 border border-white", 
