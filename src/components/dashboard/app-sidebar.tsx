...
const MANAGEMENT: SidebarItem[] = [
  ...
  },
  {
    ...
    id: 'settings',
    icon: Settings,
    items: [{ label: 'Manage Outlets', path: '/dashboard/categories' }],
  },
];
...
export function AppSidebar() {
  ...
  const [activeBranch, setActiveBranch] = useState(mockOutlets[0]);
  ...
  useEffect(() => {
    ...
    if (savedBranch) {
      try {
        ...
        const match = mockOutlets.find(b => b.id === branchData.id);
  ...
  const sortedAndFilteredBranches = useMemo(() => {
    const filtered = mockOutlets.filter((branch) =>
              ...
              <DropdownMenuPortal>
                ...
                >
                  ...
                  >
                    {!isBranchSearching ? (
                      <>
                        <DropdownMenuLabel className="p-0 text-xl font-black tracking-tight text-gray-900">Select an Outlet</DropdownMenuLabel>
                        <Button 
                          variant="ghost" 
                          size="icon" 
                    ...
                    <div className="p-2">
                      {sortedAndFilteredBranches.length > 0 ? (
                        ...
                        <div className="p-8 text-center text-sm text-muted-foreground">
                          No outlets found matching &quot;{branchSearchQuery}&quot;
                        </div>
                      )}
                    </div>
                  </ScrollArea>
